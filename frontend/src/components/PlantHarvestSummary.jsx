...
export default function PlantHarvestSummary({ data = [], year }) {
  ...
  return (
    <div className="card overflow-hidden mb-6">
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {data.map((p, i) => (
            <tr key={p.name} className="border-b border-garden-50 hover:bg-garden-50 transition-colors">
              <td className="px-5 py-3 font-medium text-garden-900">{p.name}</td>
              <td className="px-5 py-3 text-right text-garden-700">{ozToLbs(p.oz)} lbs</td>
              <td className="px-5 py-3 text-right text-garden-500">{p.count}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
